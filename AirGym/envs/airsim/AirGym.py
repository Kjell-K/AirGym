...
class AirSimMultirotorEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.low = np.array([   0.0,   0.0,   0.0, -180.0])
        self.high = np.array([100.0, 100.0, 100.0, 180.0])
        self.observation_space = spaces.Box(self.low, self.high)
        self.action_space = spaces.Discrete(4)
		
        self.goal = 	[221.0, -9.0] # global xy coordinates
        
        self.state = (100.0, 100.0, 100.0, 0.0)
    ...
    def _step(self, action):
        ...
        #append relative direction towards goal
        direction = airgym.goal_direction(self.goal, now)
        ...
        sys.stdout.write("\r\x1b[K{}/{}==>reward/depth: {:.1f}/{:.1f}   \t({:.1f}/{:.1f}/{:.1f}) {:.0f}  {:.0f}".format(self.episodeN, self.stepN, reward, rewardSum, self.state[0], self.state[1], self.state[2], direction, action))
        sys.stdout.flush()
